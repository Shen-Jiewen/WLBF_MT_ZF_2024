...
libraries/libraries/infineon_libraries/Service/CpuGeneric/SysSe/Math/%.src: ../libraries/libraries/infineon_libraries/Service/CpuGeneric/SysSe/Math/%.c libraries/libraries/infineon_libraries/Service/CpuGeneric/SysSe/Math/subdir.mk
	@echo 'Building file: $<'
	...
	if [ -f "$(basename $@).d" ]; then sed.exe -r  -e 's/\b(.+\.o)\b/libraries\/libraries\/infineon_libraries\/Service\/CpuGeneric\/SysSe\/Math\/\1/g' -e 's/\\/\//g' -e 's/\/\//\//g' -e 's/"//g' -e 's/([a-zA-Z]:\/)/\L\1/g' -e 's/\d32:/@TARGET_DELIMITER@/g; s/\\\d32/@ESCAPED_SPACE@/g; s/\d32/\\\d32/g; s/@ESCAPED_SPACE@/\\\d32/g; s/@TARGET_DELIMITER@/\d32:/g' "$(basename $@).d" > "$(basename $@).d_sed" && cp "$(basename $@).d_sed" "$(basename $@).d" && rm -f "$(basename $@).d_sed" 2>/dev/null; else echo 'No dependency file to process';fi
	@echo 'Finished building: $<'
	@echo ' '

libraries/libraries/infineon_libraries/Service/CpuGeneric/SysSe/Math/%.o: ./libraries/libraries/infineon_libraries/Service/CpuGeneric/SysSe/Math/%.src libraries/libraries/infineon_libraries/Service/CpuGeneric/SysSe/Math/subdir.mk
	@echo 'Building file: $<'
